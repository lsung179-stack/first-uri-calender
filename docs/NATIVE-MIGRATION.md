...
- 설정 탭 네이티브 광고 상세(2026-09-25): 광고 단위는 서버 `app_config` 의 `admob_native_ios` / `admob_native_android`(빈 값으로 만들어 둠 — **AdMob 에서 '네이티브 고급' 광고 단위를 만들어 넣으면 재빌드 없이 켜짐**, 비어 있으면 광고 없음). 모양은 작은 템플릿(아이콘·제목·버튼)을 설정 행 카드와 같은 색·모서리로, 위에 '광고' 라벨, '스토어' 행 아래·'기타' 라벨 위, 위아래 14px 이상. 실패는 `audit_log`(`ad_native_load_fail`). 세부: Flutter 저장소 `docs/spec/06-monetization.md` 끝 절.
