...
- `public.anniversaries` (room_id, created_by, title, date, emoji, `kind`='countup'(며칠째)|'countdown'(D-), `repeat_yearly` bool) — **방별 공유 D-day/기념일**. RLS: 방 멤버 조회+추가, 만든 사람만 수정/삭제(+관리자). 마이그레이션 `create_anniversaries`.
...
26. ✅ **D-day/기념일 (방별 공유)** — 방을 열면 캘린더 상단(`#calDdayBar`, 월표시줄과 요일헤더 사이)에 **슬림 한 줄 스트립**으로 표시(약 30px, 캘린더 공간 최소화). D-day 없으면 스트립 숨김. 탭하면 `openDdayList()`(관리 시트)→목록·추가/수정/삭제. **`kind`: 'countup'(며칠째, 첫날=1일)** / **'countdown'(D-N, 당일 D-DAY)**, `repeat_yearly` 켜면 카운트다운은 다음 주년/생일까지 계산(`_ddayNextYearly`). 임박(D-3 이하)만 강조색(`.soon`), 정렬은 `_ddayInfo().sort`(임박<사귄날3.5<먼D-day<지난것). DB 테이블 `anniversaries`(방별, RLS: 멤버조회/추가, 만든이 수정삭제). `loadDdays()`는 `enterRoom`에서 호출(방 전환 시 즉시 `_ddays=[]` 정리). 함수 전부 **전역 스코프**(renderMonthly 앞 ~8620, <16786 IIFE)라 onclick 직접 호출 OK. 상태: `_ddays`,`_ddayEditId`,`_ddayKind`,`_ddayYearly`,`_ddayDate`,`_ddayCalDate`. Playwright로 계산(100일전→101일·D-2·D-DAY)·정렬·스트립렌더·insert 저장 검증. ⚠️ **먼저 목업(artifact/스크린샷)으로 디자인 승인받고 구현**(카드형→슬림 한 줄로 축소 요청 반영). **추가시트 디자인 개선(2026-07)**: (1) **이모지 필드/선택 완전 삭제**(`DDAY_EMOJIS`·`renderDdayEmojis`·`pickDdayEmoji`·`_ddayEmoji`·`.dday-emoji-row`·`.dday-em`·`.dday-preview .pe` 제거, 스트립 pill·목록 row·미리보기에서 이모지 스팬 제거, DB `emoji` 컬럼은 미사용으로 잔존). (2) **날짜칸을 네이티브 `<input type=date>`(iOS 다크 달력, 크기 튀어나감) → 앱 일정모달과 동일한 인라인 미니 달력**(`.ev-multi-cal`/`.ev-multi-day` 재사용, `#ddayCalGrid`/`#ddayCalNavTitle`/`#ddayCalCount`)으로 교체. `renderDdayCal()`·`ddayCalNav(diff)`·`selectDdayDate(key)` 추가(`renderDayCal` 이벤트판을 그대로 미러링, `dateKey`/`parseDateKey`/`DOW_NAMES` 전역 사용). 값은 `_ddayDate`('YYYY-MM-DD')에 보관+숨김 input(`#ddayDateInput`) 동기화. Playwright로 35셀 렌더·오늘/선택 하이라이트·15일 선택→미리보기 D-8·월 네비 검증.
