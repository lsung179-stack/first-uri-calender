...
25. ✅ **방목록 순서 변경(카드 드래그)** — 방 카드를 꾹 눌러 위/아래로 끌어 재정렬. 순서는 **사용자별**이라 `members.sort_order`(int) 컬럼에 저장(마이그레이션 `add_members_sort_order`, 기기 간 동기화). `loadRooms`가 내 `members.sort_order`도 조회해 **오름차순(없으면 created_at 최신순 폴백, 안정정렬)** 정렬. `renderRooms` 카드에 `data-roomid` 부여+`_setupRoomReorder()` 호출. 드래그: `_roomDrag` 상태, `_roomPD/_roomDragStart(220ms 홀드)/_roomPM/_roomPU/_roomFinish` — 집은 카드는 `translateY`로 손가락 따라가고 나머지는 `translateY(±h)`로 자리 양보(부드럽게), 드롭 시 `_applyRoomReorder`가 `_myRooms` 재배열+낙관적 재렌더+`_saveRoomOrder`(내 members 행들에 sort_order 0..n 백그라운드 저장). 드래그 직후 카드 click(방입장) 1회 차단(`_roomSuppressClick`). 리스너는 `#roomsContent` 컨테이너 위임(재렌더에도 유지). ⚠️ 이 함수들은 전부 **바깥 스코프**(loadRooms/renderRooms 근처, <16786)라 `supa`·`_currentUser`·`toast`·`renderRooms` 직접 접근 OK. Playwright로 [A,B,C]→드래그→[B,C,A]+sort_order 저장 검증.
