...
- **코인(2026-09-23, 관리자 전용 시험 중)**: 잔액=`coin_ledger` 합계(`my_coin_balance()`), 가격=`shop_items`, 충전상품=`coin_packs`(`coins_30/100/300`), 구매=RPC **`buy_item(type,key)` 하나로만**(서버가 잔액·차감·`user_unlocks` 소유를 한 트랜잭션에서). 소유 판정 `_ownsItem(type,key)`(= `_dbUnlocks.skin/color/char`), 품목 타입 `skin`(30)·`color`/`char`/`sticker`(20)·`font`/`dh`(10). 게이트: 테마 **`_canUseSkin(k)`**, 날짜 강조 진입 **`_canDh()`**, 스티커 **`_canSticker()`** (모두 구독 OR 산 것) — 새 유료 게이트를 만들 땐 `isEffectivePremium()` 만 보지 말고 소유도 볼 것. 코인 아이콘 = 앱 로고 금화 SVG(`.coin-ico` 작은 판 / `.coin-ico-lg`·`-xl` 큰 판). 잠긴 버튼 `_lockedBtnHTML`, 구매 제안 `offerItem`, 충전 시트 `openCoinShop`. **공개 스위치 = `_coinVisible()`**(지금 `isAdmin()`). ⚠️ 공개 전 필수: (1) App Store Connect·Play Console 에 소모성 상품 `coins_30/100/300` 등록 + RevenueCat 에 추가, (2) `revenuecat-webhook` 에 코인 적립 분기(NON_RENEWING_PURCHASE→+, CANCELLATION→−, ref=거래 id) — 웹훅 비밀값을 Edge Function 시크릿으로 옮긴 뒤 배포. 관리자 테스트 코인: `select admin_grant_coins('<uid>',100,'메모');`.
