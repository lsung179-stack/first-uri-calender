...
27. ✅ **방 마스코트 (넷플릭스식, 프리미엄)** — 방 목록 씰(첫 글자)을 **방별 고유 캐릭터**로 지정. 사진 업로드가 아니라 **앱 기존 캐릭터팩 재활용**. **캐릭터당 대표 1개만 노출**(`ROOM_MASCOTS=['mochi:basic','gomdol:basic','dalkong:basic']` — 8표정 다 안 쓰고 각 1개, 사용자 요청). **전부 프리미엄 게이팅**: `openRoomMascotForRoom(id)`가 `isEffectivePremium()` 아니면 `showUpgradeModal({feature})` 페이월. **표시는 방 공유**(멤버 모두 동일, 무료 멤버도 봄 → 전환 유도) — `renderRooms` 씰이 `r.mascot` 있으면 `svgEmoji`, 없으면 첫 글자(`.r-card-seal.has-mascot`). **진입(2군데)**: (a) **방 목록 '편집' 모드**(옛 '순서 편집'을 '편집'으로 개명, `rooms>=1`이면 표시) → 각 씰에 **연필 배지(`.r-card-pencil`, `.room-edit`에서만 display) 탭** → 마스코트 시트. 연필은 `onpointerdown=stopPropagation`(드래그 방지)+capture-phase 클릭차단 리스너에서 `.r-card-pencil` 예외처리. (b) 방 설정 '방 정보'의 '방 마스코트' 행(`openRoomMascot`→`openRoomMascotForRoom(_currentRoom.id)`). 선택 시트 `#roomMascotCard`: 큰 미리보기(`#rmBigPrev`) + 대표 3종 한 줄(`.rm-grid3`/`.rm-cell2`). 값 `_mascotSel`/`_mascotRoomName`, 저장은 RPC `set_room_mascot`(방 멤버 검증, `rooms.mascot`='gomdol:basic'), 낙관적 `_currentRoom.mascot`+`_myRooms` 갱신 후 `renderRooms` 재렌더(편집모드 유지). '기본으로 되돌리기'=`clearRoomMascot`(null). 함수 전부 **전역 스코프**(`openRoomSettings` 앞 ~12504, <16786). ⚠️ 편집 모드는 **재정렬(드래그)+마스코트(연필) 겸용** — grip(≡)은 드래그, 연필은 마스코트. `if(rooms.length<1)_roomEditMode=false`(옛 `<2`에서 완화, 1방도 마스코트 편집 위해). Playwright로 편집버튼명·연필표시·무료게이트·대표3종·선택·저장·씰반영·편집모드유지 검증. ⚠️ 캐릭터팩은 원래 프리미엄 스토어 상품이라 방 마스코트도 프리미엄 유지(사용자 결정).
