...
- **코인(2026-09-23, 관리자 전용 시험 중)**: 잔액=`coin_ledger` 합계(`my_coin_balance()`), 가격=`shop_items`, 충전상품=`coin_packs`(`coins_30/100/300`, **스토어 가격 확정 C안 2026-09-24: 30코인 ₩1,500 · 110코인(100+10) ₩3,900 · 350코인(300+50) ₩9,900** — 중간 묶음=월 구독료와 같게 맞춰 코인이 구독을 깎아먹지 않게. 가격은 스토어에서만 정하고 앱은 스토어 가격 문자열을 그대로 표시), 구매=RPC **`buy_item(type,key)` 하나로만**(서버가 잔액·차감·`user_unlocks` 소유를 한 트랜잭션에서). 소유 판정 `_ownsItem(type,key)`(= `_dbUnlocks.skin/color/char`), 품목 타입 `skin`(30)·`color`/`char`/`sticker`(20)·`font`/`dh`(10). 게이트: 테마 **`_canUseSkin(k)`**, 날짜 강조 진입 **`_canDh()`**, 스티커 **`_canSticker()`** (모두 구독 OR 산 것) — 새 유료 게이트를 만들 땐 `isEffectivePremium()` 만 보지 말고 소유도 볼 것. 코인 아이콘 = **현행 앱 로고를 금 테두리 동전으로 옮긴 것**(크림 면 + 벽돌색 이중 링 + 'OUR' + 점 밑줄; `.coin-ico` 작은 판 / `.coin-ico-lg`·`-xl` 큰 판, 원본 `brand/coin*.svg`). **앱 로고는 현행 유지**(2026-09-23 재디자인 시안 전부 불채택). **스티커엔 공짜 묶음이 없다** — 다이컷 4묶음(`campnight`/`sky`/`celebrate`/`daily`, `STICKER_BASE_PACKS`)과 캐릭터 묶음 모두 구독 또는 코인 구매. 잠긴 버튼 `_lockedBtnHTML`, 구매 제안 `offerItem`, 충전 시트 `openCoinShop`. **공개 스위치 = `app_config.coin_public`**(`'true'`면 모두에게, 아니면 관리자만 — `_coinVisible()` = `_coinPublic || isAdmin()`). 충전 적립은 웹훅이 service_role RPC **`credit_coin_purchase`**/**`refund_coin_purchase`**(거래 id 로 멱등)로 한다 — 웹훅 v3 소스는 `supabase/functions/revenuecat-webhook/index.ts`(비밀값은 시크릿 `REVENUECAT_WEBHOOK_SECRET` 에서만). ⚠️ 켜기 전 필수: (1) App Store Connect·Play Console 에 소모성 상품 `coins_30/100/300` 등록 + RevenueCat 에 추가, (2) ✅ 2026-09-24 시크릿 등록 + 웹훅 코인 버전 배포 완료(Supabase 버전 4, 코드 내 기본 비밀값 제거) · 스토어 상품 3종 App Store Connect(제출 초안에 담김, 새 앱 버전과 함께 심사)·Play(활성)·RevenueCat(Consumable, Entitlement 미연결) 등록 완료. 남은 것: 새 빌드 제출 + `coin_public`='true'. 관리자 테스트 코인: `select admin_grant_coins('<uid>',100,'메모');`.
- **보상형 광고 → 코인(2026-09-24)**: 코인 충전 시트의 '광고 보고 코인 받기'(`onWatchRewardAd`, 무료 사용자·코인 공개·광고 단위 설정 시만 보임). **앱은 코인을 절대 직접 주지 않는다** — `prepareRewardVideoAd({ssv:{userId}})` → 끝까지 보면 구글이 Edge Function **`admob-ssv`**(verify_jwt=false, 구글 공개키로 ECDSA 서명 확인)로 GET → service_role RPC **`credit_ad_reward`**(ref=`admob:<transaction_id>` 멱등, **KST 하루 상한**) → `coin_ledger` kind **`'ad'`**. 앱은 잔액이 늘 때까지 확인만. 표시용 `my_ad_reward_status()`. 설정은 전부 `app_config`(재빌드 불필요): `admob_rewarded_ios`(`…/2472669914`)/`admob_rewarded_android`(`…/3775063169`)(비어 있으면 버튼 숨김), `ad_reward_coins`(1), `ad_reward_daily_cap`(10). ⚠️ iOS 플러그인은 보상 없이 닫으면 `showRewardVideoAd` Promise 를 끝내지 않으므로 결과는 리스너(`onRewardedVideoAdDismissed`/`Reward`/`FailedToShow`)로 받는다. 배너는 iOS 도 `ADAPTIVE_BANNER`.
